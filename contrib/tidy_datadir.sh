...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Hashtalt database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Hashtalt datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Hashtalt datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Hashtalt 0.7 datadir."
    ;;
  3)
    echo "Detected Hashtalt pre-0.8 datadir."
    ;;
  4)
    echo "Detected Hashtalt 0.8 datadir."
    ;;
esac
