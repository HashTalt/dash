...
const char * const BITCOIN_CONF_FILENAME = "hashtalt.conf";
const char * const BITCOIN_PID_FILENAME = "hashtaltd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "hashtalt" is a composite category enabling all Hashtalt-related debug output
                if(ptrCategory->count(std::string("hashtalt"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HashtaltCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HashtaltCore
    // Mac: ~/Library/Application Support/HashtaltCore
    // Unix: ~/.hashtaltcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HashtaltCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HashtaltCore";
#else
    // Unix
    return pathRet / ".hashtaltcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty hashtalt.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override hashtalt.conf
